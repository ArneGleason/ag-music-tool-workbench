"""Publish one candidate per visitor while the review service is stopped."""
...
specs=json.loads((job/'visitor-reference-specs.json').read_text(encoding='utf-8'))
...
for spec in specs:
 ...
 doc['assets'].append({'id':ident,'name':spec['name'],'kind':'character','brief':spec['description'],'continuity':spec['description'],'scene_ids':scenes,'versions':[{'id':'v001','image':image.name,'status':'awaiting_review','review':'','generator':'Built-in Codex image generation; exact model not specified by tool','source_ids':['CHAR-002 / v001 (style only)'],'prompt':(root/(ident+'-v001-prompt.txt')).read_text(encoding='utf-8').strip(),'sha256':hashlib.sha256(image.read_bytes()).hexdigest()}]})
